...
def inicializate_video(name_video):
    ...
    out_center = cv2.VideoWriter(name, fourcc, 60.0, (1920,1080))
    
    #Right View Point
    name = name_video+'_right.mp4'
    fourcc = cv2.VideoWriter_fourcc(*'MP4V')
    out_right = cv2.VideoWriter(name, fourcc, 60.0, (1920,1080))
    
    #Left View Point
    name = name_video+'_left.mp4'
    fourcc = cv2.VideoWriter_fourcc(*'MP4V')
    out_left = cv2.VideoWriter(name, fourcc, 60.0, (1920,1080))
...
class BasicSynchronousClient(object):
    ...
    def setup_camera(self):
        ...
        camera_transform_c = carla.Transform(carla.Location(x=410.4, y=-104.0, z=7.5), carla.Rotation(pitch=-20,  yaw=-90, roll=0.000000)) #pattern
        camera_transform_r = carla.Transform(carla.Location(x=420.4, y=-104.0, z=7.5), carla.Rotation(pitch=-15,  yaw=-115, roll=0.000000)) #pattern 2
        camera_transform_l = carla.Transform(carla.Location(x=395.4, y=-104.0, z=7.5), carla.Rotation(pitch=-15,  yaw=-48, roll=0.000000))
    ...
    #         if self.show:
    #             surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
    #             # Create a surface from the center image array
    #             center_surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
    #             # Blit the center surface to the top left corner of the main surface
    ...
    #        if self.show:
    #            # Create a surface from the right image array
    #            right_surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
    #            # Blit the right surface to the top right corner of the main surface
    #            surface.blit(right_surface, (display.get_width() // 2, 0))
    ...
    #        if self.show:
    #            # Create a surface from the left image array
    #            left_surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
    #            # Blit the left surface to the bottom left corner of the main surface
    #            surface.blit(left_surface, (0, display.get_height() // 2))
    ...
    def game_loop(self, args):
        ...
        try:
            ...
            #FIXME - NOVO
            self.region_speed = self.convert3DRegionsSpeed_to_2d(SPEED_ESTIMATION_REGION_3D, self.camera_l.E, self.camera_l.K)
            ...
            while self.frame_number < total_fps:
                ...
                self.frame_number+=1
                print("FRAME NUMBER: "+str(self.frame_number), end='\r')
